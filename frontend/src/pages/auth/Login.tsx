...
export default function Login() {
  ...
  React.useEffect(() => {
    const checkBootstrap = async () => {
      try {
        const res = await api.get(`/auth/bootstrap/${isRevenue ? 'sdo' : moduleType}`);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Revenue module accepts the same credentials as SDO (any authenticated user)
      formData.append('module_type', isRevenue ? 'sdo' : (moduleType || ''));
      ...
      else if (isRevenue) navigate('/revenue');
  ...
  return (
    <div className={`min-h-screen flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden ${isSDO ? 'bg-slate-50' : isQuery ? 'bg-emerald-50/50' : isApis ? 'bg-violet-50/50' : isRevenue ? 'bg-teal-50/50' : 'bg-amber-50/50'}`}>
      ...
      {isRevenue && (
        <>
          ...
          <div className="absolute bottom-[-10%] left-[-10%] w-[600px] h-[600px] bg-cyan-500/10 rounded-full -z-10"></div>
        </>
      )}
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md cursor-pointer" onClick={() => navigate('/modules')}>
        <div className="flex justify-center flex-col items-center group">
          <div className={`p-4 rounded-2xl shadow-lg border transition-transform group-hover:scale-105 ${isSDO ? 'bg-blue-900 border-blue-800' : isQuery ? 'bg-emerald-900 border-emerald-800' : isApis ? 'bg-violet-900 border-violet-800' : isRevenue ? 'bg-teal-800 border-teal-700' : 'bg-amber-800 border-amber-700'}`}>
            ...
            {isRevenue && <ClipboardList className="w-12 h-12 text-teal-300" />}
          </div>
          <h2 className={`mt-6 text-center text-3xl font-extrabold tracking-tight ${isSDO ? 'text-blue-900' : isQuery ? 'text-emerald-900' : isApis ? 'text-violet-900' : isRevenue ? 'text-teal-900' : 'text-amber-900'}`}>
            {isSDO ? 'SDO MODULE' : isQuery ? 'QUERY MODULE' : isApis ? 'COPS ↔ APIS' : isRevenue ? 'REVENUE REPORT' : 'ADJUDICATION MODULE'}
          </h2>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md relative z-10 mb-10">
        <div className={`py-8 px-4 shadow-2xl sm:rounded-xl sm:px-10 border ${isSDO ? 'bg-white/80 border-blue-100' : isQuery ? 'bg-white/80 border-emerald-100' : isApis ? 'bg-white/80 border-violet-100' : isRevenue ? 'bg-white/80 border-teal-100' : 'bg-white/80 border-amber-200'}`}>
          ...
          <form className="space-y-6" onSubmit={handleLogin}>
            ...
            <div className="pt-2">
              ...
              >
                {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : `Log In to ${isSDO ? 'SDO' : isQuery ? 'Query' : isApis ? 'COPS ↔ APIS' : isRevenue ? 'Revenue Report' : 'Adjudication'}`}
              </button>
            </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 flex justify-center items-center py-3">
        <p className={`text-xs tracking-widest uppercase select-none ${isSDO ? 'text-slate-400/60' : isQuery ? 'text-emerald-900/30' : isApis ? 'text-violet-900/30' : isRevenue ? 'text-teal-900/30' : 'text-amber-900/30'}`}>
          Powered by{' '}
          <span className={`font-semibold ${isSDO ? 'text-slate-500/70' : isQuery ? 'text-emerald-900/50' : isApis ? 'text-violet-900/50' : isRevenue ? 'text-teal-900/50' : 'text-amber-900/50'}`}>
