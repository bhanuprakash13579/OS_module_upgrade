...
export default function RevenueSheet({ session: initialSession, rules, onMessage, onConfig, onRulesChanged }: Props) {
   ...
   * receipt it is about — "Add item to BR 49773" rather than an unlabelled mark,
  ...
  const [formulaEdit, setFormulaEdit] = useState<
    { rule: DrFormulaRule; expression: string; username: string; password: string; busy: boolean; error: string | null } | null>(null);
  ...
  const duplicateRow = useCallback((idx: number) => {
    setEntries(prev => {
      ...
      if (!from) return prev;
      const next = [...prev];
      next.splice(idx + 1, 0, { ...from, _uid: nextUid(), id: undefined, br_no: '', os_ref: '' });
      return next;
    });
    scheduleAutoSave();
  ...
  const addSubItem = useCallback((afterIdx: number) => {
    setEntries(prev => {
      ...
      // The same receipt was paid the same way. Nothing else is carried over:
      // the value and the duty belong to the item, not to the receipt.
      sub.is_sbi_challan = parent.is_sbi_challan;
      next.splice(afterIdx + 1, 0, sub);
      return next;
    });
    scheduleAutoSave();
    // straight onto the item, which is the only thing left to type
    const itemCol = ENTRY_COLS.findIndex(c => c.key === 'item_desc');
    setTimeout(() => cellRefs.current.get(`${afterIdx + 1}-${itemCol}`)?.focus(), 0);
  }, [scheduleAutoSave]);
  ...
  const downloadRevenue = async () => {
    ...
    if (!ok) return;
    try {
      const res = await api.get(`/dcr/sessions/${initialSession.id}/download/revenue`, {
        responseType: 'blob',
      });
      const fname = `${fmtDate(session.report_date)} REVENUE REPORT.xlsx`;
      const url = URL.createObjectURL(res.data);
      const a = document.createElement('a');
      a.href = url; a.download = fname; a.click();
      URL.revokeObjectURL(url);
            ...
            <div className="flex justify-end gap-2 px-5 py-3 bg-slate-50 border-t border-slate-100">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    await api.post(
                      `/dcr/formula-rules/${formulaEdit.rule.id}/revise`,
                      { expression: formulaEdit.expression,
                        username: formulaEdit.username, password: formulaEdit.password });
                    // Pick it up straight away, without reopening the sheet.
                    onRulesChanged?.();
                    setFormulaEdit(null);
                  } catch (err: unknown) {
                    const detail = (err as { response?: { data?: { detail?: string } } })
                      ?.response?.data?.detail || 'Could not update the formula.';
