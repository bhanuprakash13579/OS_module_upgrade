...
export default function AdjudicatedList() {
  ...
  const fetchCases = useCallback(async (page: number, search: string, year = filterYear) => {
    ...
    setErrorMsg('');
    try {
      const params: Record<string, any> = { page, per_page: PER_PAGE, search: search.trim(), status: 'adjudicated' };
  ...
  useEffect(() => { fetchCases(1, ''); setCurrentPage(1); }, []); // eslint-disable-line react-hooks/exhaustive-deps

  const handleSearchChange = (value: string) => {
    setSearchTerm(value);
    clearTimeout(searchDebounce.current);
    searchDebounce.current = setTimeout(() => { setCurrentPage(1); fetchCases(1, value); }, 400);
  };
  ...
  return (
    <div className="space-y-6 flex flex-col max-w-7xl mx-auto pt-2 pb-12">
      ...
      <div className="flex justify-between items-center bg-green-700 text-white p-5 rounded-xl shadow-md border border-green-600">
        ...
        <button onClick={() => { setCurrentPage(1); fetchCases(1, searchTerm); }} className="flex items-center gap-2 bg-green-600 hover:bg-green-500 border border-green-400 px-4 py-2 rounded-lg text-sm transition-colors">
        ...
        {showFilter && (
          <div className="mt-4 pt-4 border-t border-slate-200 flex flex-wrap gap-4 items-end">
            ...
            <div className="flex gap-2">
              <button onClick={() => { setCurrentPage(1); fetchCases(1, searchTerm, filterYear); setShowFilter(false); }}
              ...
              <button onClick={() => { setFilterYear(''); setCurrentPage(1); fetchCases(1, searchTerm, ''); setShowFilter(false); }}
