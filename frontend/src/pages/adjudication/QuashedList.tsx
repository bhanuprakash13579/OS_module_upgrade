...
export default function QuashedList() {
  ...
  const fetchCases = useCallback(async (page: number, search: string, year = filterYear) => {
    setLoading(true);
    try {
      const params: Record<string, any> = { page, per_page: PER_PAGE, search: search.trim(), status: 'quashed' };
  ...
  return (
    <div className="space-y-6 h-full flex flex-col max-w-7xl mx-auto pt-2">
      ...
      <div className="flex justify-between items-center bg-slate-700 text-white p-5 rounded-xl shadow-md border border-slate-600">
        ...
        <button onClick={() => { setCurrentPage(1); fetchCases(1, searchTerm); }} className="flex items-center gap-2 bg-slate-600 hover:bg-slate-500 border border-slate-500 px-4 py-2 rounded-lg text-sm transition-colors">
