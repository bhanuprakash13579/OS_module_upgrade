...
export default function OSQueryPage() {
  ...
  const [activeSearch, setActiveSearch] = useState({ ...formData } as typeof formData);
  ...
  // Build export payload from the last executed search
    ...
    try {
      ...
      const rows = allRows.map(r => {
        const itemsSummary = (r.items || [])
          .map((i: any) => `${i.items_desc || ''}${i.items_qty != null ? ` x ${i.items_qty}` : ''}${i.items_uqc ? ` ${i.items_uqc}` : ''}`.trim())
          .join('; ');
        return [
          ...
          ...(showItemDesc ? [`"${((r.items || []).map((i: any) => i.items_desc).filter(Boolean).join('; ')).replace(/"/g, '""')}"`] : []),
                    ...
                    return (
                      <React.Fragment key={idx}>
                        <tr className="hover:bg-slate-50 transition-colors break-inside-avoid">
                          ...
                          {showItemDesc && (
                            <td className="px-4 py-3">
                              <div className="text-sm text-slate-700 truncate max-w-[200px]" title={(r.items || []).map(i => i.items_desc).filter(Boolean).join('; ')}>
                                {(r.items || []).map(i => i.items_desc).filter(Boolean).join('; ') || '—'}
                              </div>
                            </td>
                          )}
                          <td className="px-4 py-2 align-top bg-amber-50/30">
                            {(r.items || []).length === 0 ? (
                              ...
                              <div className="space-y-0.5">
                                {(r.items || []).map((item: any, ii: number) => (
