...
def generate_report(
    ...
    if report_id == "r4":  # B.R. Register
        ...
        records = db.query(BrMaster).filter(BrMaster.br_date >= start_date, BrMaster.br_date <= end_date, BrMaster.entry_deleted != "Y").limit(ROW_LIMIT).all()
        for r in records:
            writer.writerow([
        ...
        records = db.query(CopsMaster).filter(CopsMaster.os_date >= start_date, CopsMaster.os_date <= end_date, CopsMaster.entry_deleted != "Y").limit(ROW_LIMIT).all()
        for r in records:
            # Status must match _pending_filters() in offence.py
            _status = 'Adjudicated' if (r.adjudication_date or r.adj_offr_name) else ('Quashed' if r.quashed == 'Y' else ('Rejected' if r.rejected == 'Y' else 'Pending'))
            writer.writerow([
        ...
        records = db.query(DrMaster).filter(DrMaster.dr_date >= start_date, DrMaster.dr_date <= end_date, DrMaster.entry_deleted != "Y").limit(ROW_LIMIT).all()
        for r in records:
            writer.writerow([
